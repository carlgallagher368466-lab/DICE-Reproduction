...
class ContextManager(object):
    ...
    def set_load_path(flags_obj):
        ...
        elif flags_obj.dataset == 'ml10m_longtail_test':
            flags_obj.load_path = const_util.ml10m_longtail_test
        elif flags_obj.dataset == 'ml10m_head_test':
            flags_obj.load_path = const_util.ml10m_head_test
        elif flags_obj.dataset == 'ml10m_uniform_test':
            flags_obj.load_path = const_util.ml10m_uniform_test
        elif flags_obj.dataset == 'ciao':
            flags_obj.load_path = const_util.ciao
        elif flags_obj.dataset == 'epinions':
            flags_obj.load_path = const_util.epinions
    ...
    def set_trainer(flags_obj, cm, vm, dm):
        ...
        if flags_obj.model == 'VAE':
            return trainer.VAEPairTrainer(flags_obj, cm, vm, dm)
        elif flags_obj.model == 'VAEIPS':
            return trainer.VAEIPSPairTrainer(flags_obj, cm, vm, dm)
        elif 'IPS' in flags_obj.model:
    ...
    def set_recommender(flags_obj, workspace, dm):
        ...
        elif flags_obj.model == 'IDICE':
            return recommender.IDICERecommender(flags_obj, workspace, dm)
        elif flags_obj.model == 'IPS':
            return recommender.IPSRecommender(flags_obj, workspace, dm)
        elif flags_obj.model == 'NeuMF':
            return recommender.NeuMFRecommender(flags_obj, workspace, dm)
        elif flags_obj.model == 'NeuMFIPS':
            return recommender.NeuMFIPSRecommender(flags_obj, workspace, dm)
        elif flags_obj.model == 'NeuMFDICE':
            return recommender.NeuMFDICERecommender(flags_obj, workspace, dm)
        elif flags_obj.model == 'NGCF':
            return recommender.NGCFRecommender(flags_obj, workspace, dm)
        elif flags_obj.model == 'NGCFIPS':
            return recommender.NGCFIPSRecommender(flags_obj, workspace, dm)
        elif flags_obj.model == 'NGCFDICE':
            return recommender.NGCFDICERecommender(flags_obj, workspace, dm)
        elif flags_obj.model == 'VAE':
            return recommender.VAERecommender(flags_obj, workspace, dm)
        elif flags_obj.model == 'VAEIPS':
            return recommender.VAEIPSRecommender(flags_obj, workspace, dm)
        elif flags_obj.model == 'VAEDICE':
            return recommender.VAEDICERecommender(flags_obj, workspace, dm)
...
class VizManager(object):
    ...
    def __init__(self, flags_obj):
        ...
        self.use_visdom = flags_obj.use_visdom
        self.set_visdom()
    ...
    def set_visdom(self):
        ...
        if Visdom is None:
            self.viz = NullVisdom()
            return
        viz = Visdom(port=self.port, env=self.exp_name)
        if hasattr(viz, 'check_connection') and not viz.check_connection():
            self.viz = NullVisdom()
            return
        self.viz = viz
...
class DICESampler(SAMPLER.Sampler):
    ...
    def generate_negative_samples(self, user, pos_item):
        ...
        fallback_items = np.setdiff1d(np.arange(len(self.popularity)), np.asarray(user_pos, dtype=np.int64), assume_unique=False)

        def sample_candidate(candidates, count):
            if len(candidates) == 0:
                candidates = fallback_items
            if len(candidates) == 0:
                raise ValueError('No negative candidates available for user {}'.format(user))
        ...
        if num_pop_items < self.pool:
            ...
            for count in range(self.neg_sample_rate):
                ...
                item = sample_candidate(unpop_items, count)
            ...
            for count in range(self.neg_sample_rate):
                ...
                item = sample_candidate(pop_items, count)
