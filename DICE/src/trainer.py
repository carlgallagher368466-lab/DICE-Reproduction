...
class Trainer(object):
    ...
    def train(self):
        ...
        for epoch in range(self.flags_obj.epochs):
            ...
            self.recommender.cleanup_ckpt(self.max_epoch)
...
class VAEPairTrainer(PairTrainer):

    def get_loss(self, sample, batch_count):

        p_score, n_score, kl_loss = self.recommender.pair_inference(sample)

        self.distances[batch_count] = (p_score - n_score).mean().item()

        loss = self.pair_loss(p_score, n_score) + self.flags_obj.vae_kl_weight * kl_loss

        return loss


class VAEIPSPairTrainer(IPSPairTrainer):

    def get_loss(self, sample, batch_count):

        p_score, n_score, weight, kl_loss = self.recommender.pair_inference(sample)

        self.distances[batch_count] = (p_score - n_score).mean().item()

        loss = self.pair_loss(p_score, n_score, weight) + self.flags_obj.vae_kl_weight * kl_loss
